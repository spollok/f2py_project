...
!---------------------------------------------------------------------------------------!
    ...
    subroutine loadMagTile( tile, centerPos, dev_center, rect_size, Mag, u_ea, u_oa1, u_oa2, mu_r_ea, mu_r_oa, Mrem, tileType, offset, rotAngles, color, magnetType, stateFunctionIndex, includeInIteration, exploitSymmetry, symmetryOps, Mrel )
        ...
        tile%r0 = centerPos(1)
        tile%theta0 = centerPos(2)
        tile%z0 = centerPos(3)
        tile%dr = dev_center(1)
        tile%dtheta = dev_center(2)
        tile%dz = dev_center(3)
        tile%a = rect_size(1)
        tile%b = rect_size(2)
        tile%c = rect_size(3)
        tile%M = Mag
        ...
        tile%u_oa1 = u_oa1
        tile%u_oa2 = u_oa2
        tile%mu_r_ea = mu_r_ea
        tile%mu_r_oa = mu_r_oa
        tile%Mrem = Mrem
        tile%tileType = tileType
        tile%offset = offset
        tile%rotAngles = rotAngles
        tile%color = color
        tile%magnetType = magnetType
        tile%stateFunctionIndex = stateFunctionIndex
        tile%includeInIteration = includeInIteration
        tile%exploitSymmetry = exploitSymmetry
        tile%symmetryOps = symmetryOps
        tile%Mrel = Mrel
