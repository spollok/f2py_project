...
class Tile():
        def __init__(self):
                ...
                self.center_pos = np.zeros(shape=(3), dtype=np.float32, order='F') # r0, theta0, z0
                self.dev_center = np.zeros(shape=(3), dtype=np.float32, order='F') # dr, dtheta, dz
                self.size = np.zeros(shape=(3), dtype=np.float32, order='F') # a, b, c
                self.M = np.zeros(shape=(3), dtype=np.float32, order='F') # Mx, My, Mz
                self.u_ea = np.array([1, 0, 0], dtype=np.float32, order='F') # Easy axis
                self.u_oa1 = np.array([0, 1, 0], dtype=np.float32, order='F')
                self.u_oa2 = np.array([0, 0, 1], dtype=np.float32, order='F')
                self.M_rem = 0.
                self.mu_r_ea = 1.
                self.mu_r_oa = 1.
                ...
                self.offset = np.zeros(shape=(3), dtype=np.float32, order='F') # offset of global coordinates
                self.rot = np.zeros(shape=(3), dtype=np.float32, order='F')
                self.color = np.array([1, 0, 0], dtype=np.float32, order='F')
                self.use_sym = 1 # whether to exploit symmetry
                self.sym_op = np.array([1, 1, 1], dtype=np.float32, order='F') # 1 for symmetry and -1 for anti-symmetry respectively to the planes
                self.M_rel = 0.
...
class Grid():
        ...
        def set_eval_points(self, n_points, mode):
                # points = []
                counter = 0
                points = np.zeros(shape=(n_points[0]*n_points[1]*n_points[2],3), dtype=np.float32, order='F')
                if mode == "uniform":
